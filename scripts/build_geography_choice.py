...
def pdf_text(contents: bytes) -> str:
    try:
        ...
        raise RuntimeError("pdftotext is required to build Geography practice data") from exc
...
def pdf_bbox_pages(contents: bytes) -> list[PdfPage]:
    try:
        ...
        raise RuntimeError("pdftotext is required to build Geography source images") from exc
    except subprocess.CalledProcessError as exc:
        message = exc.stderr.decode("utf-8", errors="replace")
...
def png_dimensions(contents: bytes) -> tuple[int, int]:
    if contents[:8] != b"\x89PNG\r\n\x1a\n":
        raise ValueError("Rendered Geography source image is not a PNG")
        ...
        for page_number, page_crops in sorted(crops_by_page.items()):
            ...
            if image_path.is_file() and not force_render:
                ...
                try:
                    ...
                    raise RuntimeError("pdftocairo is required to build Geography source images") from exc
                except subprocess.CalledProcessError as exc:
                    message = exc.stderr.decode("utf-8", errors="replace")
