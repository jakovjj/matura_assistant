#!/usr/bin/env python3
"""Build the interactive History practice index from mirrored NCVVO ZIP files."""
...
OUTPUT = ROOT / "data" / "history-choice.js"
PAPER_ROOT = ROOT / "files" / "interactive" / "history-choice"
PAPER_URL_PREFIX = "./files/interactive/history-choice"
ARCHIVE_PREFIX = "window.ASISTENT_ZA_MATURE_DATA="
OUTPUT_PREFIX = "window.ASISTENT_ZA_MATURE_HISTORY_CHOICE="
SUBJECT = "Povijest"
...
TASK_LABELS = {
    ...
    "kronologija": "Zadatci višestrukoga izbora (kronologije)",
    "uz-polazni-sadrzaj": "Zadatci višestrukoga izbora (uz polazni sadržaj)",
...
ENTRY_RE = re.compile(r"^\s*(?P<number>\d{1,3}(?:[\.,]\d{1,2})?)(?:\.|\s+)(?P<answer>.*)$")
...
def exam_id(exam: dict[str, Any]) -> str:
    return f"povijest-{exam['year']}-{slugify(normalize_term(exam['term']))}"
...
def history_question_token(line: PdfLine) -> str | None:
...
def history_marker_token(line: PdfLine) -> str | None:
    token = history_question_token(line)
    if token is None:
        return None
...
def history_first_task_page(pages: list[PdfPage]) -> PdfPage | None:
    return first_page_containing(pages, r"\bI\.\s+Zadat")


def find_history_question_markers(
    pages: list[PdfPage],
    wanted_questions: set[str],
) -> dict[str, QuestionMarker]:
    first_task_page = history_first_task_page(pages)
    ...
    for page in pages:
        ...
        for line in sorted_page_lines(page):
            token = history_marker_token(line)
...
def running_header_bottom(page: PdfPage) -> float:
    header_lines = [
        ...
        if line.y_min < 130
        and re.fullmatch(r"(?:Povijest|POVIJEST)", line.text.strip(), flags=re.IGNORECASE)
    ]
...
def find_history_question_crops(markers: dict[str, QuestionMarker]) -> dict[str, QuestionCrop]:
...
def task_id_for_heading(label: str) -> str:
    ...
    if "kratkoga odgovora" in normalized:
        return "kratki-odgovor"
    if "kronolog" in normalized:
        return "kronologija"
    if "polazni" in normalized:
        return "uz-polazni-sadrzaj"
    if "visestrukoga izbora" in normalized:
...
def clean_prompt_text(text: str) -> str:
    ...
    for raw_line in text.splitlines():
        ...
        if re.fullmatch(r"_+", line):
            continue
        if re.fullmatch(r"(?:Povijest|POV(?: D-S\d+)?|\d+)", line):
            continue
        if re.search(r"^(?:POV|Nacionalni centar|NCVVO)\b", line):
            continue
        lines.append(line)
...
def build_tasks(entries: list[dict[str, Any]], paper_text: str, sections: list[PaperSection]) -> tuple[list[dict[str, Any]], dict[str, list[str]], dict[str, dict[str, Any]]]:
    ...
    for entry in entries:
        ...
        if answer:
            ...
            if task_id in {"visestruki-izbor", "kronologija", "uz-polazni-sadrzaj"}:
    ...
    with tempfile.TemporaryDirectory() as temporary_directory:
        ...
        for page_number, page_crops in sorted(crops_by_page.items()):
            ...
            for key, crop in page_crops:
                ...
                x_min, y_min, x_max, y_max = refine_history_crop_box(
                    page_image,
                    crop,
                    x_min,
                    y_min,
...
def build_source_images(
    ...
    markers = find_history_question_markers(pages, wanted_numbers)
    crops = find_history_question_crops(markers)
    all_crops = {
        **{
...
def build_exam(exam: dict[str, Any]) -> dict[str, Any]:
    ...
    if not answers and not open_answers:
        raise ValueError("Could not parse any History answers")
...
def main() -> None:
    ...
    print(f"Wrote {len(exams)} History practice exams to {OUTPUT.relative_to(ROOT)}")
    if skipped:
        print(f"Skipped {len(skipped)} History archives without reliable generated data")
