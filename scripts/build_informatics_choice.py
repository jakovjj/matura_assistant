#!/usr/bin/env python3
"""Build the interactive Informatics practice index from mirrored NCVVO ZIP files."""
...
OUTPUT = ROOT / "data" / "informatics-choice.js"
PAPER_ROOT = ROOT / "files" / "interactive" / "informatics-choice"
PAPER_URL_PREFIX = "./files/interactive/informatics-choice"
ARCHIVE_PREFIX = "window.ASISTENT_ZA_MATURE_DATA="
OUTPUT_PREFIX = "window.ASISTENT_ZA_MATURE_INFORMATICS_CHOICE="
SUBJECT = "Informatika"
...
TASK_LABELS = {
    ...
    "skupina-zadataka": "Otvoreni zadatci",
    "otvoreni-zadaci": "Zadatci otvorenoga tipa",
}
TASK_DESCRIPTIONS = {
    "visestruki-izbor": "Odaberi jedan točan odgovor za svako pitanje.",
    "visestruke-kombinacije": "Označi sve točne odgovore za svako pitanje.",
    "povezivanje": "Poveži svaki ponuđeni pojam s odgovarajućim brojem iz zadatka.",
    ...
    "skupina-zadataka": "Riješi zadatke uz likovne primjere i ručno dodijeli bodove prema službenom rješenju.",
    "otvoreni-zadaci": "Riješi zadatak, otvori službeno rješenje ako je dostupno i dodijeli si bodove.",
}
TASK_ORDER = [
    "visestruki-izbor",
...
ENTRY_RE = re.compile(
    r"^\s*(?P<number>\d{1,3}(?:[\.,]\d{1,2})?)(?:\.|\s+)(?P<answer>.*)$",
    flags=re.IGNORECASE,
)
PAPER_QUESTION_RE = re.compile(r"(?m)^\s*(?P<number>\d{1,3}(?:\.\d{1,2})?)\.\s+")
PAPER_HEADING_RE = re.compile(r"(?im)^\s*(?P<roman>[IVX]+)\.\s+(?P<label>(?:Zadat|Skupina)[^\n]+)")
ANSWER_LETTER_RE = re.compile(r"^\s*(?P<answer>[A-Fa-f])(?:\s*$|[\.)]\s*|\s+)")
ANSWER_LETTERS_RE = re.compile(r"(?<![A-Za-z])(?P<answer>[A-Fa-f])(?=\s*(?:[.,;)]|$))")
...
MATCHING_ASSIGNMENT_RE = re.compile(
    r"(?<!\d)(?P<item>\d{1,2})\s*[.\-:]?\s*(?P<answer>[A-Fa-f])\.?(?=\s*[,;/]|\s*$)"
)
...
def exam_id(exam: dict[str, Any]) -> str:
    return f"informatika-{exam['year']}-{slugify(normalize_term(exam['term']))}"
...
def pdf_text(contents: bytes, *args: str) -> str:
    return pdftotext(
        contents,
        *(args or ("-layout",)),
        required_message="pdftotext is required to build Informatics practice data",
    )
...
def pdf_bbox_pages(contents: bytes) -> list[PdfPage]:
    xml = pdftotext(
        contents,
        "-bbox-layout",
        required_message="pdftotext is required to build Informatics source images",
...
def primary_booklet_text(text: str) -> str:
    cut_patterns = (
        r"(?im)^\s*ANALIZA\s+LIKOVNOGA\s+PRIMJERA\s*$",
        r"(?im)^\s*(?:KLJU[ČC]\s+ZA\s+ODGOVORE\s+[-–]\s+)?ISPITNA\s+KNJI[ŽZ]ICA\s+2\s*$",
        r"(?im)^\s*II\.\s+Ispitna\s+cjelina\s+[-–]\s+Esej\s*$",
    )
    cut_points = [
        match.start()
...
def sociology_question_token(line: PdfLine) -> str | None:
...
def sociology_marker_token(line: PdfLine) -> str | None:
    token = sociology_question_token(line)
    if token is None:
        return None
...
def sociology_first_task_page(pages: list[PdfPage]) -> PdfPage | None:
    return first_page_containing(pages, r"\bI\.\s+Zadat")


def find_sociology_question_markers(
    pages: list[PdfPage],
    wanted_questions: set[str],
) -> dict[str, QuestionMarker]:
    first_task_page = sociology_first_task_page(pages)
    ...
    for page in pages:
        ...
        for line in sorted_page_lines(page):
            token = sociology_marker_token(line)
...
def running_header_bottom(page: PdfPage) -> float:
    header_lines = [
        ...
        if line.y_min < 130
        and re.fullmatch(r"(?:Informatika|INF)", line.text.strip(), flags=re.IGNORECASE)
    ]
...
def question_crop_y_max(marker: QuestionMarker, ordered_markers: list[QuestionMarker]) -> float:
    ...
    if len(relevant_lines) == 1 and sociology_question_token(relevant_lines[0]) == marker.number:
...
def find_sociology_question_crops(markers: dict[str, QuestionMarker]) -> dict[str, QuestionCrop]:
...
def is_key_name(name: str) -> bool:
    if is_macos_metadata_name(name):
        return False
...
def find_paper_names(names: list[str]) -> list[str]:
    ...
    if first_booklets and second_booklets:
        return [sorted(first_booklets, key=lambda name: normalized_name(Path(name).name))[0]]
...
def combine_pdf_contents(parts: list[bytes]) -> bytes:
    ...
    with tempfile.TemporaryDirectory() as temporary_directory:
        ...
        try:
            ...
            raise RuntimeError("pdfunite is required to combine Informatics exam booklets") from exc
...
def canonicalize_pdf_id(contents: bytes, source_parts: list[bytes]) -> bytes:
    ...
    if not matches:
        raise ValueError("Combined Informatics PDF does not contain a trailer ID")
...
def question_sort_key(question: str) -> tuple[int, int, str]:
    match = re.match(r"^(?P<whole>\d+)(?:\.(?P<suffix>\d+|[A-Z]))?$", str(question))
    if not match:
        return (9999, 9999, str(question))
    suffix = match.group("suffix") or ""
    decimal = int(suffix) if suffix.isdigit() else 0
    letter = suffix if suffix and not suffix.isdigit() else ""
    return (int(match.group("whole")), decimal, letter)


def task_id_for_heading(label: str) -> str:
    normalized = unicodedata.normalize("NFD", label.casefold()).encode("ascii", "ignore").decode()
    if "skupina" in normalized:
        return "skupina-zadataka"
...
def parse_key_entries(key_texts: list[str], sections: list[PaperSection]) -> list[dict[str, Any]]:
    ...
    for key_text in key_texts:
        ...
        def flush() -> None:
            ...
            if not answer_text:
                # Informatics short answers are often a single exact value
                # (a pure integer such as "32" or a binary string). The generic
                # key cleaner skips standalone-number lines to drop stray page
                # numbers, which would otherwise discard these legitimate
                # answers. Keep a single short value as the answer text instead.
                cleaned = [clean_line(line) for line in current["lines"]]
                cleaned = [line for line in cleaned if line]
                if len(cleaned) == 1 and len(cleaned[0]) <= 40:
                    answer_text = cleaned[0]
            # Informatics keys label subparts as "26.1.: 1536"; strip the
            # leading colon that survives the number token.
            answer_text = re.sub(r"^\s*:\s*", "", answer_text)
            if answer_text or current.get("keepEmpty"):
                entries.append(
                    {
        ...
        for raw_line in key_text.splitlines():
            ...
            heading_task_id = (
                task_id_for_heading(line)
                if ("zadat" in line.casefold() or "skupina" in line.casefold())
                else None
            )
...
def remove_parent_source_entries(entries: list[dict[str, Any]]) -> list[dict[str, Any]]:
    child_parents = {
        str(entry["number"]).split(".", 1)[0]
        for entry in entries
        if "." in str(entry["number"])
    }
    return [
        entry
        for entry in entries
        if not (
            str(entry["number"]) in child_parents
            and re.match(
                r"^\s*(?:Izvor(?:i)?|Prilagođeno prema)\b",
                str(entry.get("answerText") or ""),
                flags=re.IGNORECASE,
            )
        )
    ]
...
def closed_answers(answer_text: str) -> list[str]:
    text = str(answer_text or "")
    assignments = list(MATCHING_ASSIGNMENT_RE.finditer(text))
    if assignments:
        return []
    letters = [match.group("answer").upper() for match in ANSWER_LETTERS_RE.finditer(text)]
    unique: list[str] = []
...
def matching_assignments(answer_text: str) -> dict[str, str]:
    assignments: dict[str, str] = {}
    for match in MATCHING_ASSIGNMENT_RE.finditer(str(answer_text or "")):
        target = str(int(match.group("item")))
        letter = match.group("answer").upper()
        assignments[letter] = target
    return assignments
...
def clean_prompt_text(text: str) -> str:
    ...
    for raw_line in text.splitlines():
        ...
        if re.fullmatch(r"_+", line):
            continue
        if re.fullmatch(r"(?:Informatika|INF(?: D-S\d+)?|\d+)", line):
            continue
        if re.search(r"^(?:LIU|Nacionalni centar|NCVVO)\b", line):
            continue
        lines.append(line)
...
def build_tasks(entries: list[dict[str, Any]], paper_text: str, sections: list[PaperSection]) -> tuple[list[dict[str, Any]], dict[str, list[str]], dict[str, dict[str, Any]]]:
    ...
    for entry in entries:
        question = str(entry["number"])
        answer_text = str(entry.get("answerText") or "").strip()
        if "." not in question and not answer_text and question in child_parents:
            continue

        section = section_for_question(sections, question)
        task_id = entry.get("taskId") or (section.task_id if section else None) or "otvoreni-zadaci"
        ...
        assignments = matching_assignments(answer_text)
        if task_id == "povezivanje" and len(assignments) >= 2:
            target_labels = sorted(set(assignments.values()), key=lambda value: int(value))
            scored_items = sorted(assignments, key=lambda value: (ord(value), value))
            question_item = {
                "number": question,
                "type": "matching",
                ...
                "maxPoints": len(assignments),
            }
            for letter, target in assignments.items():
                answers[f"{question}.{letter}"] = [target]
        else:
            letters = closed_answers(answer_text)
            if task_id == "visestruke-kombinacije" and len(letters) >= 2:
                question_item = {
                    "number": question,
                    "type": "multi-choice",
                    "options": ["A", "B", "C", "D"],
                    "maxPoints": len(letters),
                }
                answers[question] = letters
            elif len(letters) == 1:
                question_item = {"number": question}
                if letters[0] in {"E", "F"}:
                    question_item["options"] = ["A", "B", "C", "D", "E", "F"]
                answers[question] = letters
            else:
...
def refine_sociology_crop_box(
    ...
    image_only_question = (
        ...
        and sociology_question_token(visible_content_lines[0]) is not None
    )
        ...
        for page_number, page_crops in sorted(crops_by_page.items()):
            ...
            if image_path.is_file() and not force_render:
                ...
                render_pdf_page_to_png(
                    ...
                    page_number,
                    SOURCE_RENDER_DPI,
                    required_message="pdftocairo is required to build Informatics source images",
                )
                contents = temporary_prefix.with_suffix(".png").read_bytes()
            image_width, image_height = png_dimensions(
                contents,
                error_message="Rendered Informatics source image is not a PNG",
                strict_ihdr=False,
            )
            ...
            for key, crop in page_crops:
                ...
                x_min, y_min, x_max, y_max = refine_sociology_crop_box(
                    page_image,
                    crop,
                    x_min,
                    y_min,
...
def build_source_images(
    ...
    markers = find_sociology_question_markers(pages, wanted_numbers)
    crops = find_sociology_question_crops(markers)
    all_crops = {
        **{
...
def build_exam(exam: dict[str, Any]) -> dict[str, Any]:
    ...
    with zipfile.ZipFile(archive_path) as archive:
        ...
        key_texts = [text for text in key_texts if text.strip()]

    paper_text = primary_booklet_text(pdf_text(paper_contents))
    ...
    entries = remove_parent_source_entries(entries)
    ...
    # Crop official-solution images for every open task, not only the ones with
    # a parsed model answer: drawing subitems (such as 28.1) keep their answer
    # as an annotated image in the key and otherwise show "no solution".
    open_question_set = {
        str(question["number"])
        for task in tasks
        for question in task.get("questions", [])
    ...
    try:
        solution_images = build_manual_solution_images(
            ...
            required_message="pdftocairo is required to build Informatics solution images",
            wanted_questions=open_question_set,
            group_aware=True,
        )
    ...
    result = {
        ...
        "openAnswers": {
            question: open_answers[question]
            for question in open_questions
            if question in open_answers
        },
    }
    return result
...
def main() -> None:
    ...
    print(f"Wrote {len(exams)} Informatics practice exams to {OUTPUT.relative_to(ROOT)}")
    if skipped:
        print(f"Skipped {len(skipped)} Informatics archives without reliable generated data")
