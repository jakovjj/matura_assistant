#!/usr/bin/env python3
"""Build the interactive Art practice index from mirrored NCVVO ZIP files."""
...
OUTPUT = ROOT / "data" / "art-choice.js"
PAPER_ROOT = ROOT / "files" / "interactive" / "art-choice"
PAPER_URL_PREFIX = "./files/interactive/art-choice"
ARCHIVE_PREFIX = "window.ASISTENT_ZA_MATURE_DATA="
OUTPUT_PREFIX = "window.ASISTENT_ZA_MATURE_ART_CHOICE="
...
def exam_id(exam: dict[str, Any]) -> str:
    return f"likovna-umjetnost-{exam['year']}-{slugify(normalize_term(exam['term']))}"
...
def pdf_text(contents: bytes, *args: str) -> str:
    return pdftotext(
        contents,
        *(args or ("-layout",)),
        required_message="pdftotext is required to build Art practice data",
    )
...
def pdf_bbox_pages(contents: bytes) -> list[PdfPage]:
    xml = pdftotext(
        contents,
        "-bbox-layout",
        required_message="pdftotext is required to build Art source images",
...
def running_header_bottom(page: PdfPage) -> float:
    header_lines = [
        ...
        if line.y_min < 130
        and re.fullmatch(r"(?:Likovna umjetnost|LIU)", line.text.strip(), flags=re.IGNORECASE)
    ]
...
def combine_pdf_contents(parts: list[bytes]) -> bytes:
    ...
    with tempfile.TemporaryDirectory() as temporary_directory:
        ...
        try:
            ...
            raise RuntimeError("pdfunite is required to combine Art exam booklets") from exc
...
def canonicalize_pdf_id(contents: bytes, source_parts: list[bytes]) -> bytes:
    ...
    if not matches:
        raise ValueError("Combined Art PDF does not contain a trailer ID")
...
def clean_prompt_text(text: str) -> str:
    ...
    for raw_line in text.splitlines():
        ...
        if re.fullmatch(r"_+", line):
            continue
        if re.fullmatch(r"(?:Likovna umjetnost|LIU(?: D-S\d+)?|\d+)", line):
            continue
        ...
        for page_number, page_crops in sorted(crops_by_page.items()):
            ...
            if image_path.is_file() and not force_render:
                ...
                render_pdf_page_to_png(
                    ...
                    page_number,
                    SOURCE_RENDER_DPI,
                    required_message="pdftocairo is required to build Art source images",
                )
                contents = temporary_prefix.with_suffix(".png").read_bytes()
            image_width, image_height = png_dimensions(
                contents,
                error_message="Rendered Art source image is not a PNG",
                strict_ihdr=False,
            )
...
def build_exam(exam: dict[str, Any]) -> dict[str, Any]:
    ...
    if not answers and not open_answers:
        raise ValueError("Could not parse any Art answers")
    ...
    try:
        solution_images = build_manual_solution_images(
            ...
            required_message="pdftocairo is required to build Art solution images",
...
def main() -> None:
    ...
    print(f"Wrote {len(exams)} Art practice exams to {OUTPUT.relative_to(ROOT)}")
    if skipped:
        print(f"Skipped {len(skipped)} Art archives without reliable generated data")
