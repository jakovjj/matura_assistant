#!/usr/bin/env python3
"""Build the interactive Philosophy practice index from mirrored NCVVO ZIP files."""
...
OUTPUT = ROOT / "data" / "philosophy-choice.js"
PAPER_ROOT = ROOT / "files" / "interactive" / "philosophy-choice"
PAPER_URL_PREFIX = "./files/interactive/philosophy-choice"
ARCHIVE_PREFIX = "window.ASISTENT_ZA_MATURE_DATA="
OUTPUT_PREFIX = "window.ASISTENT_ZA_MATURE_PHILOSOPHY_CHOICE="
SUBJECT = "Filozofija"
# Badly OCR-scanned archives whose official answer key text is too corrupted to
# parse reliable open-answer boundaries (garbled glyphs such as "7.t."). They are
# skipped instead of shipping unreliable model answers; the strict open-answer
# validator stays active for every other exam.
KNOWN_UNRELIABLE_IDS = {
    "filozofija-2015-ljetni-rok",
    "filozofija-2015-jesenski-rok",
}
...
TASK_LABELS = {
    "visestruki-izbor": "Višestruki izbor",
    "visestruke-kombinacije": "Višestruke kombinacije",
    "povezivanje": "Povezivanje",
    "kratki-odgovor": "Zadatci kratkoga odgovora",
    "produzeni-odgovor": "Zadatak produženoga odgovora",
    "otvoreni-zadaci": "Otvoreni zadatci",
}
# kratki-odgovor and produzeni-odgovor are merged into a single "Otvoreni zadatci"
# category for display (see merge_open_tasks); their individual labels above are
# only used if the merge step is ever skipped.
MERGED_OPEN_TASK_IDS = ("kratki-odgovor", "produzeni-odgovor", "otvoreni-zadaci")
TASK_DESCRIPTIONS = {
...
def exam_id(exam: dict[str, Any]) -> str:
    return f"filozofija-{exam['year']}-{slugify(normalize_term(exam['term']))}"
...
def pdf_text(contents: bytes, *args: str) -> str:
    return pdftotext(
        contents,
        *(args or ("-layout",)),
        required_message="pdftotext is required to build Philosophy practice data",
    )
...
def pdf_bbox_pages(contents: bytes) -> list[PdfPage]:
    xml = pdftotext(
        contents,
        "-bbox-layout",
        required_message="pdftotext is required to build Philosophy source images",
...
def philosophy_question_token(line: PdfLine) -> str | None:
...
def philosophy_marker_token(line: PdfLine) -> str | None:
    token = philosophy_question_token(line)
    if token is None:
        return None
...
def philosophy_first_task_page(pages: list[PdfPage]) -> PdfPage | None:
    return first_page_containing(pages, r"\bI\.\s+Zadat")


def find_philosophy_question_markers(
    pages: list[PdfPage],
    wanted_questions: set[str],
) -> dict[str, QuestionMarker]:
    first_task_page = philosophy_first_task_page(pages)
    ...
    for page in pages:
        ...
        for line in sorted_page_lines(page):
            token = philosophy_marker_token(line)
...
def running_header_bottom(page: PdfPage) -> float:
    header_lines = [
        ...
        if line.y_min < 130
        and re.fullmatch(r"(?:Filozofija|FIL)", line.text.strip(), flags=re.IGNORECASE)
    ]
...
def question_crop_y_max(marker: QuestionMarker, ordered_markers: list[QuestionMarker]) -> float:
    ...
    if len(relevant_lines) == 1 and philosophy_question_token(relevant_lines[0]) == marker.number:
...
def find_philosophy_question_crops(markers: dict[str, QuestionMarker]) -> dict[str, QuestionCrop]:
...
def combine_pdf_contents(parts: list[bytes]) -> bytes:
    ...
    with tempfile.TemporaryDirectory() as temporary_directory:
        ...
        try:
            ...
            raise RuntimeError("pdfunite is required to combine Philosophy exam booklets") from exc
...
def canonicalize_pdf_id(contents: bytes, source_parts: list[bytes]) -> bytes:
    ...
    if not matches:
        raise ValueError("Combined Philosophy PDF does not contain a trailer ID")
...
def clean_prompt_text(text: str) -> str:
    ...
    for raw_line in text.splitlines():
        ...
        if re.fullmatch(r"_+", line):
            continue
        if re.fullmatch(r"(?:Filozofija|FIL(?: D-S\d+)?|\d+)", line):
            continue
        if re.search(r"^(?:FIL|Nacionalni centar|NCVVO)\b", line):
            continue
        lines.append(line)
...
def refine_philosophy_crop_box(
    image: Image.Image,
    crop: QuestionCrop,
    x_min: int,
    y_min: int,
    ...
    image_only_question = (
        ...
        and philosophy_question_token(visible_content_lines[0]) is not None
    )
        ...
        for page_number, page_crops in sorted(crops_by_page.items()):
            ...
            if image_path.is_file() and not force_render:
                ...
                render_pdf_page_to_png(
                    ...
                    page_number,
                    SOURCE_RENDER_DPI,
                    required_message="pdftocairo is required to build Philosophy source images",
                )
                contents = temporary_prefix.with_suffix(".png").read_bytes()
            image_width, image_height = png_dimensions(
                contents,
                error_message="Rendered Philosophy source image is not a PNG",
                strict_ihdr=False,
            )
            ...
            for key, crop in page_crops:
                ...
                x_min, y_min, x_max, y_max = refine_philosophy_crop_box(
                    page_image,
                    crop,
                    x_min,
                    y_min,
...
def build_source_images(
    ...
    markers = find_philosophy_question_markers(pages, wanted_numbers)
    crops = find_philosophy_question_crops(markers)
    all_crops = {
        **{
...
def is_matching_item(question: dict[str, Any]) -> bool:
    number = str(question["number"])
    return bool(
        "." in number
        and question.get("options")
        and question.get("type") != "open"
        and not question.get("excluded")
    )
    ...
    for task in tasks:
        ...
        for question in task["questions"]:
            if not is_matching_item(question):
                ...
                continue
            parent, _, item = str(question["number"]).partition(".")
            matching = groups.get(parent)
            if matching is None:
                matching = {
                    "number": parent,
                    "type": "matching",
...
def merge_open_tasks(tasks: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Merge the short- and extended-answer tasks into one "Otvoreni zadatci" tab."""
    if not any(task["id"] in MERGED_OPEN_TASK_IDS for task in tasks):
        return tasks
    merged_questions = [
        question
        for task in tasks
        if task["id"] in MERGED_OPEN_TASK_IDS
        for question in task["questions"]
    ]
    merged_task = {
        "id": "otvoreni-zadaci",
        "label": TASK_LABELS["otvoreni-zadaci"],
        "description": TASK_DESCRIPTIONS["otvoreni-zadaci"],
...
def build_exam(exam: dict[str, Any]) -> dict[str, Any]:
    ...
    if not answers and not open_answers:
        raise ValueError("Could not parse any Philosophy answers")
    ...
    solution_images = build_manual_solution_images(
        ...
        required_message="pdftocairo is required to build Philosophy solution images",
    )
...
def main() -> None:
    ...
    for exam in candidate_exams:
        identifier = exam_id(exam)
        if identifier in KNOWN_UNRELIABLE_IDS:
            skipped.append(identifier)
            print(f"warning: skipped {identifier}: known unreliable scan", file=sys.stderr)
            continue
        try:
    ...
    print(f"Wrote {len(exams)} Philosophy practice exams to {OUTPUT.relative_to(ROOT)}")
    if skipped:
        print(f"Skipped {len(skipped)} Philosophy archives without reliable generated data")
