const data = window.ASISTENT_ZA_MATURE_SOCIOLOGY_CHOICE;

if (!data || !Array.isArray(data.exams)) {
  throw new Error("Nedostaje generirani indeks interaktivnih zadataka iz Sociologije.");
}

const app = document.querySelector("#sociology-app");
...
function sociologyExamIdForTerm(exam, term) {
  return `sociologija-${exam.year}-${slugPart(term)}`;
}

function sociologyStorageKeyForId(id) {
  return `asistent-za-mature:sociology-choice:${id}`;
}

function sociologyStorageKeys(exam) {
  const ids = [
    exam.id,
    ...(legacyTermAliases[exam.term] || []).map((term) => sociologyExamIdForTerm(exam, term)),
  ];

  return [...new Set(ids)].map(sociologyStorageKeyForId);
}
...
function buildExamMap(items) {
  ...
  for (const exam of items) {
    ...
    for (const legacyTerm of legacyTermAliases[exam.term] || []) {
      map.set(sociologyExamIdForTerm(exam, legacyTerm), exam);
    }
  }
  return map;
}
...
const exams = data.exams.map((exam) => {
  ...
  return {
    ...exam,
    term,
    id: sociologyExamIdForTerm(exam, term),
  };
});
...
function sociologySubjectUrl() {
  return "./?predmet=Sociologija";
}
...
function loadState(exam) {
  ...
  try {
    for (const key of sociologyStorageKeys(exam).reverse()) {
...
function saveState() {
  ...
  try {
    localStorage.setItem(
      sociologyStorageKeyForId(solverExam.id),
      JSON.stringify({
        closedResponses,
        openResponses,
        openScores,
...
function renderMissingExam() {
  ...
  app.innerHTML = `
    <div class="empty-state">
      ...
      <p>Odabrani ispit iz Sociologije nije dostupan.</p>
      <a class="start-link" href="${sociologySubjectUrl()}">Vrati se na Sociologiju</a>
    </div>
  `;
}
...
function startSociologyPage() {
  const id = selectedExamId();
  if (!id) {
    window.location.replace(sociologySubjectUrl());
    return;
  }
...
if (!simulation.active && window.AsistentProfile?.ready) {
  window.AsistentProfile.ready.finally(startSociologyPage);
} else {
  startSociologyPage();
}
