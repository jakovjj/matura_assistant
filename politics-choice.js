...
function allQuestions(exam = solverExam) {
  return sourceTasks(exam).flatMap(scoredTaskQuestions);
}
...
function resolvedScore() {
  return closedScore(true) + resolvedOpenScore();
}
...
function bindResponseListeners() {
  ...
  document.querySelectorAll("[data-open-question]").forEach((input) => {
    input.addEventListener("input", () => {
      updateOpenResponse(input.dataset.openQuestion, input.value);
      const container = input.closest(".history-open-question");
...
function quickSelectAnswerState(question) {
  if (isExcludedQuestion(question)) return "izuzet iz bodovanja";
  if (isOpenQuestion(question)) {
    return openResponses[question]?.trim() || hasOpenScore(question)
      ? "odgovoreno"
      : "nije odgovoreno";
  }
...
function quickSelectIsAnswered(question) {
  if (isExcludedQuestion(question)) return false;
  return isOpenQuestion(question)
    ? Boolean(openResponses[question]?.trim()) || hasOpenScore(question)
    : Boolean(closedResponses[question]);
}
...
function renderQuickSelect() {
  ...
  quickSelect.innerHTML = questions
    .map((question) => {
      const stateClass = isExcludedQuestion(question)
        ? " question-quickselect__link--excluded"
        : quickSelectIsAnswered(question)
...
function renderCroppedImage(source, alt) {
  return window.renderSourceImageCrop(source, alt, { loading: "eager" });
}
