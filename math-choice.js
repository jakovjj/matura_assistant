const data = window.ASISTENT_ZA_MATURE_MATH_CHOICE;

if (!data || !Array.isArray(data.exams)) {
  throw new Error("Nedostaje generirani indeks interaktivnih zadataka iz Matematike.");
}

const app = document.querySelector("#math-app");
...
function choiceStorageKeyForId(id) {
  return `asistent-za-mature:math-choice:${id}`;
}
...
function mathSubjectUrl() {
  return "./?predmet=Matematika";
}
...
function renderMissingExam() {
  document.body.classList.remove("solver-page", "math-solver-page");
  app.classList.remove("math-solver-active");
  app.innerHTML = `
    <div class="empty-state">
      <h2>Ispit nije pronađen</h2>
      <p>Odabrani ispit iz Matematike nije dostupan.</p>
      <a class="start-link" href="${mathSubjectUrl()}">Vrati se na Matematiku</a>
    </div>
  `;
}
...
function startMathPage() {
  const id = selectedExamId();
  if (!id) {
    window.location.replace(mathSubjectUrl());
    return;
  }
...
if (!simulation.active && window.AsistentProfile?.ready) {
  window.AsistentProfile.ready.finally(startMathPage);
} else {
  startMathPage();
}
